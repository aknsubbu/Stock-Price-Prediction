...
def check_volatility(self, required_ticks=1000, consistency_ticks=500, volatility_threshold=0.15):
    ...
    if high_volatility_ticks > consistency_ticks:
        print(f"Volatility has stayed above {volatility_threshold * 100}% for more than {consistency_ticks} out of {required_ticks} ticks. Warning issued.")
...
def establishTrendDirection(ticker, short_required_ticks: int = 10, long_required_ticks: int = 100):
    def get_trend_direction():
        short_trend = 1 if ticker.short_mavg.iloc[-1] > ticker.short_mavg.iloc[-2] else 0
    ...
    short_consistent_trend = check_consistent_trend_direction(short_required_ticks, [short_trend, long_trend])
    long_consistent_trend = check_consistent_trend_direction(long_required_ticks, [short_trend, long_trend])
    ...
    if short_consistent_trend:
        print(f"The short-term trend has been consistently {'Bullish' if short_trend == 1 else 'Bearish'} for at least {short_required_ticks} price ticks.")
    else:
        print(f"The short-term trend is not consistent for the required number of ticks.")

    if long_consistent_trend:
        print(f"The long-term trend has been consistently {'Bullish' if long_trend == 1 else 'Bearish'} for at least {long_required_ticks} price ticks.")
    else:
        print(f"The long-term trend is not consistent for the required number of ticks.")

    return short_trend, long_trend


def check_crossover_and_warning(self, short_required_ticks=150, long_required_ticks=500, percent_threshold=5):
    crossover_warning = False
    percent_threshold += 100

    short_crossover_condition = self.history['Close'].iloc[-1] > percent_threshold * self.short_mavg.iloc[-1] and \
                                self.history['Close'].iloc[-2] <= percent_threshold * self.short_mavg.iloc[-2]


    long_crossover_condition = self.history['Close'].iloc[-1] > percent_threshold * self.long_mavg.iloc[-1] and \
                               self.history['Close'].iloc[-2] <= percent_threshold * self.long_mavg.iloc[-2]
    ...
    short_price_above_threshold = (self.history['Close'].iloc[-short_required_ticks:] > percent_threshold * self.short_mavg.iloc[-short_required_ticks:]).all()
    long_price_above_threshold = (self.history['Close'].iloc[-long_required_ticks:] > percent_threshold * self.long_mavg.iloc[-long_required_ticks:]).all()

    if short_price_above_threshold or long_price_above_threshold:
        print(f"The price has been consistently 5% above the moving average for the last {short_required_ticks} ticks (short-term). Warning issued.")
        print(f"The price has been consistently 5% above the moving average for the last {long_required_ticks} ticks (long-term). Warning issued.")
        crossover_warning = True
...
def predict_based_on_rsi(self, overbought_threshold=70, oversold_threshold=30):
    ...
    if current_rsi < oversold_threshold:
        print(f"RSI is below {oversold_threshold} (oversold region). Potential increase is likely. RSI: {current_rsi}")
        rsi_prediction = 1

    elif current_rsi > overbought_threshold:
        print(f"RSI is above {overbought_threshold} (overbought region). Potential decrease is likely. RSI: {current_rsi}")
        rsi_prediction = 0

    else:
        print(f"RSI is between {oversold_threshold} and {overbought_threshold}. No clear signal. RSI: {current_rsi}")
...
def make_cumulative_prediction(short_trend, long_trend, crossover_warning, volatility_warning, rsi_prediction):

    cumulative_prediction = (short_trend + long_trend + crossover_warning + volatility_warning + rsi_prediction) >= 3
